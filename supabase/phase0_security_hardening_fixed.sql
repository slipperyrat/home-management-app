-- Phase 0: Security Hardening & Advanced Features (Fixed Version)
-- This script enhances security, adds rate limiting, and strengthens RLS policies
-- Fixed to work without pg_cron extension
...
-- 10. MANUAL CLEANUP FUNCTION (can be called manually or via external scheduler)
-- Note: Without pg_cron, you'll need to call this manually or set up an external cron job
...
-- 11. INSERT SOME TEST DATA TO VERIFY THE SYSTEM WORKS
INSERT INTO security_events (user_id, event_type, severity, details, ip_address, user_agent)
VALUES 
    ('test_user_123', 'system.test', 'low', '{"test": true}', '127.0.0.1', 'test-agent')
...
BEGIN
    ...
    RAISE NOTICE 'Run: SELECT manual_cleanup(); to clean up old data manually.';
    RAISE NOTICE 'Note: Set up external cron job to call manual_cleanup() every hour for automatic cleanup.';
END $$;
