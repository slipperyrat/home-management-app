-- Fix conflict detection functions to use correct data types
-- The issue is that attendee_user_id is text, not uuid

-- Drop and recreate the functions with correct types
DROP FUNCTION IF EXISTS detect_calendar_conflicts(uuid);
DROP FUNCTION IF EXISTS upsert_calendar_conflicts(uuid);
DROP FUNCTION IF EXISTS check_event_conflicts(uuid);

-- Function to detect conflicts for a specific household (fixed types)
CREATE OR REPLACE FUNCTION detect_calendar_conflicts(p_household_id uuid)
RETURNS TABLE (
  event_id_a uuid,
  event_id_b uuid,
  attendee_user_id text,  -- Changed from uuid to text
  conflict_type text,
  severity text
) AS $$
BEGIN
...
-- Function to upsert conflicts into calendar_conflicts table (fixed types)
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT (household_id, event1_id, event2_id) DO NOTHING;
  END LOOP;
END;
$$ LANGUAGE plpgsql;

-- Function to check conflicts for a specific event (for real-time detection) (fixed types)
...
BEGIN
  ...
  LOOP
    -- Insert conflict (using the smaller ID as event1_id for consistency)
    INSERT INTO calendar_conflicts (
      household_id,
      event1_id,
      event2_id,
...
COMMENT ON FUNCTION detect_calendar_conflicts(uuid) IS 'Detects calendar conflicts for overlapping events with same attendee in a household (fixed types)';
COMMENT ON FUNCTION upsert_calendar_conflicts(uuid) IS 'Updates calendar_conflicts table with current conflicts for a household (fixed types)';
COMMENT ON FUNCTION check_event_conflicts(uuid) IS 'Checks for conflicts when a specific event is created or updated (fixed types)';
