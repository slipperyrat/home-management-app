-- Add conflict detection function for overlapping events with same attendee
-- This implements the "Option 1.5" approach: basic but real conflict detection
...
-- Function to upsert conflicts into calendar_conflicts table
...
-- Function to check conflicts for a specific event (for real-time detection)
...
COMMENT ON FUNCTION detect_calendar_conflicts(uuid) IS 'Detects calendar conflicts for overlapping events with same attendee in a household';
COMMENT ON FUNCTION upsert_calendar_conflicts(uuid) IS 'Updates calendar_conflicts table with current conflicts for a household';
COMMENT ON FUNCTION check_event_conflicts(uuid) IS 'Checks for conflicts when a specific event is created or updated';
