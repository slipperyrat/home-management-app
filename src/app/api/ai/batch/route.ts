...
import { batchProcessor, BatchRequest, BatchJob } from '@/lib/ai/services/BatchProcessor';

export async function POST(request: NextRequest) {
  return withAPISecurity(request, async (req, user) => {
    try {
      console.log('📦 POST: Batch processing request for user:', user.id);
      ...
      switch (action) {
        case 'create_job':
          ...
          const batchRequests: Omit<BatchRequest, 'id' | 'createdAt' | 'retryCount'>[] = requests.map((req: any) => ({
            type: req.type,
            context: req.context,
        ...
        case 'cancel_job':
          ...
          const cancelled = batchProcessor.cancelBatchJob(cancelJobId);
...
export async function GET(request: NextRequest) {
  return withAPISecurity(request, async (req, user) => {
    try {
      console.log('📦 GET: Batch processing data for user:', user.id);
      ...
      switch (action) {
        case 'overview':
          ...
          const userJobs = allJobs.filter(job => 
            job.requests.some(req => req.userId === user.id)
          );
        ...
        case 'job':
          ...
          const hasAccess = job.requests.some(req => req.userId === user.id);
          if (!hasAccess) {
        ...
        case 'queue':
          ...
          const userQueue = queue.filter(req => req.userId === user.id);
